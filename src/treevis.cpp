...
int main(int argc , char *argv[])
{
  try
  {
    ...
    else if (std::string(argv[1]) == "-e" && argc > 4)
    {
      int id_end = std::stoi(argv[2], NULL);
      double width = std::stod(argv[3], NULL);
      double hight = std::stod(argv[4], NULL);
      TreeManager tree(FILE_NAME_NEW, FILE_NODE_END);
      tree.ShowNodeEnd(id_end, width, hight); 
    }
...
void WriteUsage()
{
  ...
  std::cout << "treevis -e [node's id_end] [window's width] [window's hight]\n";
